...
router.post("/searchUser",Constants.checkAccessMiddleware, function (req, res) {
    res.header('Access-Control-Allow-Origin', '*');
    res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
    var result = new Object();
    ...
    if (keyword.match(format)) {
        ...
        User.find((err, users) => {
            if (err) {
                ...
                let usrList = users.filter(function (singleUser) { return singleUser.name.includes(keyword) || singleUser.email.includes(keyword) });
                if(usrList.length==0)
                {
                    result.success=true;
                ...
                else
                {
                    ...
                    usrList.forEach(u => {
                        ...
                        obj.email = u.email;
                        obj.name = u.name;
                        obj.no_of_followings = u.followers.followingsList.length;
                        obj.no_of_followers = u.followers.followersList.length;
                        obj.profile_pic = u.profile_pic !== undefined ? u.profile_pic : "null";
                        result.users.push(obj);
                    });
                    result.success = true;
                    result.message = result.users.length +" users found";
                    res.status(Constants.CODE_OK).json(result);
                }           
            }
        })
...
// 
router.post("/getUserProfile",Constants.checkAccessMiddleware, function (req, res) {
    res.header('Access-Control-Allow-Origin', '*');
    res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
    ...
    if (userId.match(format)) {
        ...
        User.findOne(criteria).then(u => {
          ...
          obj.email = u.email;
          obj.name = u.name;
          obj.following = u.followers.followingsList;
          obj.followers = u.followers.followersList;
          obj.profile_pic = u.profile_pic !== undefined ? u.profile_pic : "null";
          obj.driveAccountsList = new Object();
          obj.shared_content =[];
          obj.date_created = u.CreatedOn;
          result.userProfile = obj;
